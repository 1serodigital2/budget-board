...
export const addCategory = async ({ userId, categoryDetail, }) => {
    try {
...
export const getCategoryById = async ({ userId, categoryId, }) => {
    try {
        ...
        const docSnap = await getDoc(doc(db, `users/${userId}/category`, categoryId));
        if (docSnap.exists()) {
...
export const deleteCategory = async ({ userId, categoryId, }) => {
    try {
        ...
        const categoryRef = collection(db, `users/${userId}/category`);
        const uncategorizedQuery = query(categoryRef, where("isSystem", "==", true), where("slug", "==", "uncategorized"), limit(1));
        ...
        const updatePromises = expenseSnapshot.docs.map((expenseDoc) => updateDoc(expenseDoc.ref, {
...
export const updateCategory = async ({ userId, catId, categoryDetail, }) => {
    try {
        const normalizedName = categoryDetail.category.trim().toLowerCase();
        const slug = normalizedName.replace(/\s+/g, "-");
...
export const createDefaultCategories = async (uid) => {
    try {
        ...
        await Promise.all(defaultCategories.map((category) => addDoc(collection(db, `users/${uid}/category`), category)));
    }
    catch (error) {
