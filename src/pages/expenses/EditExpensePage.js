...
const EditExpensePage = () => {
    const { inputValues, handleInputChange, submitMessage, setInputValues, showSubmitMessage, getExpenseDetail, } = useExpenseForm();
    const { user } = useAuth();
    const { id } = useParams();
    ...
    if (isLoading) {
        return _jsx(Alert, { message: "Loading expense" });
    }
    if (isError) {
        return (_jsx(Alert, { type: "error", message: error.message || "Unable to get expense detail" }));
    }
    ...
    return (_jsxs(_Fragment, { children: [_jsx("h1", { className: "text-2xl font-medium mb-4", children: "Edit Expense" }), _jsx(ExpenseForm, { handleFormSubmit: handleFormSubmit, handleInputChange: handleInputChange, inputValues: inputValues, isPending: isPending, submitMessage: submitMessage })] }));
};
